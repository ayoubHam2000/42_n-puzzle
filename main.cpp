...
void   initialize_graph(Node *node, std::vector<int> &maze, std::vector<bool> &visited, int size)
{
  int cell = node->get_data();
  visited[cell] = true;

  if ((maze[cell] & 0b1000) && visited[cell] == false)
    node->add_neighbor(new Node(maze[cell - size]));
  if ((maze[cell] & 0b0100) && visited[cell] == false)
    node->add_neighbor(new Node(maze[cell + 1]));
  if ((maze[cell] & 0b0010) && visited[cell] == false)
    node->add_neighbor(new Node(maze[cell + size]));
  if ((maze[cell] & 0b0001) && visited[cell] == false)
    node->add_neighbor(new Node(maze[cell - 1]));
  for (auto n : node->get_neighbors()) {
    initialize_graph(node, maze, visited, size);
  }
}
...
Node  *initialize_graph(Global *g)
{
  ...
  initialize_graph(root, g->maze, visited, g->maze_size);
  return root;
}
